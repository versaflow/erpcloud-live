...
class InstanceAccessMiddleware
{
    ...
    try {
        if($request->route()->getActionName() != 'App\Http\Controllers\CoreController@getLogout'){
            if(!session('instance')->customer_erp && !empty(session('role_id')) && !empty(session('role_level'))){
        ...
        //auto login admin if logged in main instance
         if (!session('instance')->customer_erp && empty(session('user_id')) && !empty($request->admin_user_id)) {
