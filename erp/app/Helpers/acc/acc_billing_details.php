...
function button_subscription_summary_manage($request)
{
    ...
    if ('pbx_extension' == $provision_type || 'sip_trunk' == $provision_type) {
     ...
     if ('phone_number' == $provision_type) {
    ...
    if ('hosting' == $provision_type || 'sitebuilder' == $provision_type) {
    ...
    if ('virtual_server' == $provision_type) {
        $subs = \DB::table('sub_services')->where('status','!=','Deleted')->where('product_id',$summary->product_id)->get();
        foreach($subs as $s){
    ...
    if ('fibre' == $provision_type || 'fibre_product' == $provision_type) {
    ...
    if ('lte_sim_card' == $provision_type) {
        $subs = \DB::table('sub_services')->where('status','!=','Deleted')->where('product_id',$summary->product_id)->get();
        foreach($subs as $s){
    ...
    if ('mtn_lte_sim_card' == $provision_type || 'telkom_lte_sim_card' == $provision_type) {
        $subs = \DB::table('sub_services')->where('status','!=','Deleted')->where('product_id',$summary->product_id)->get();
        foreach($subs as $s){
    ...
    if ('airtime_prepaid' == $provision_type || 'airtime_unlimited' == $provision_type || 'airtime_contract' == $provision_type) {
