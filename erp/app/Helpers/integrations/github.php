...
function get_github_commits_daily($repo = false, $branch = 'master', $since = false){
    ...
    $token = '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';
    $username = 'versaflow';
    $repo = 'erpcloud';
    
    $client = new \Github\Client();
    $auth = $client->authenticate($token, '', Github\AuthMethod::ACCESS_TOKEN);
    $commits = $client->api('repo')->commits()->all($username, $repo, array('sha' => 'master'));
    // vd($commits);
    if ($commits) {
        return $commits;
    } else {
