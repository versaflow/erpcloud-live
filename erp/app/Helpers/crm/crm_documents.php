...
function beforesave_check_stock_availability($request)
{
    ...
    for ($i = 0; $i < count($products); ++$i) {
        if (empty($request->qty[$i]) || 0 == $request->qty[$i]) {
...
function void_transaction($transaction_table, $transaction_id, $doctype, $complete_transaction = false, $revert = false)
{
    ...
    if ('crm_documents' == $transaction_table || 'crm_supplier_documents' == $transaction_table) {
        $line_id = 'document_id';
    }


    $transaction = \DB::table($transaction_table)->where('id', $transaction_id)->get()->first();
    if ('crm_supplier_documents' == $transaction_table && $revert) {
        \DB::table($transaction_table)->where('id', $transaction_id)->update(['doctype' => 'Supplier Order']);
        return $transaction_id;
    }


    if ('Order' == $doctype || 'Quotation' == $doctype  || 'Credit Note Draft' == $doctype) {
        $description = '';
        if (!empty($line_id)) {
            // copy lines
    ...
    if (strstr($transaction_table, '_documents')) {
        ...
        if ('Tax Invoice' == $transaction->doctype || 'Supplier Invoice' == $transaction->doctype) {
            ...
            if (!empty($transaction) && !empty($transaction->total)) {
                ...
                if ('crm_documents' == $transaction_table) {
                    $transaction->completed = 1;
                    $transaction->credit_note_reason = 'Credited from Tax Invoice #'.$transaction->id;
                }

                if ('crm_supplier_documents' == $transaction_table) {
...
{   
    ...
    if ($total < 0 && ('customer' == $account->type)) {
        $total = abs($total);
