...
function get_branding_pricelist_links()
{
    $list = [];
    $i = 20000;
    $db_storefronts = \DB::connection('default')->table('crm_shopify_integrations')->select('name', 'logo', 'id', 'helpdesk_email', 'email_template')->get();
...
{
    $db_storefront = \DB::table('crm_shopify_integrations')->where('id', $storefront_id)->select('name', 'logo', 'id', 'helpdesk_email', 'email_template')->get()->first();
