...
function schedule_daily_pbx_disk_usage()
{
    ...
    $percent_disk_usage = '';
    foreach ($tmp as $stat) {
        if (substr_count($stat, '%') > 0) {
            $percent_disk_usage = rtrim($stat, '%');
            break;
        }
    }
...
function pbx_call($dial_number, $id, $type = 'account', $outbound_caller_id = false)
{
    ...
    */
    ...
    // $click_to_call_number = \DB::connection('system')->table('crm_shopify_integrations')->where('instance_id',session('instance')->id)->pluck('phone_number')->first();
...
*/
        ...
        foreach ($airtime_invoices as $inv) {
            if (date('Y-m-d', strtotime($call->hangup_time)) >= $inv->docdate && $inv->docdate > $first_record_date) {
                if (count($invoice_ids) == 0 || ! in_array($inv->document_id, $invoice_ids)) {
