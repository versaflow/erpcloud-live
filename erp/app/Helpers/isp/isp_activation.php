...
function provision_invoices($account_id)
{
    ...
    if ('Deleted' != $customer->status && ('customer' == $customer->type || 'reseller_user' == $customer->type)) {
    ...
    if ('reseller' == $customer->type && 1 != $customer->id) {
...
function provision_auto($document_id)
{
    ...
    // vd($document);
    if ($document && 'Tax Invoice' == $document->doctype) {
...
function create_provision_items($invoice)
{
    ...
    // aa($invoice_line);
        ...
        if (147 == $invoice_line->product_id) {
            continue;
        }
...
function provision_checklist_status($provision, $customer)
{
    ...
    foreach ($items as $item) {
        ...
        if ('' != trim($item_id)) {
            ...
            if (!empty($checklist_status[$item_id]) && '1' == $checklist_status[$item_id]) {
                continue;
            } else {
                return false;
            }
