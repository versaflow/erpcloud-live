...
function provision_sitebuilder_form($provision, $input, $product, $customer)
{
    ...
    if (! empty($input['domain_action'])) {
        $register_selected = ($input['domain_action'] == 'Register') ? 'selected="selected"' : '';
        $transfer_selected = ($input['domain_action'] == 'Transfer') ? 'selected="selected"' : '';
    }
...
function provision_mtn5g_lte_sim_card_form($provision, $input, $product, $customer)
{
  ...
  'postalofusage' => 'Gauteng',
    ...
    */
    ...
    $form .= 'Address:<br><input placeholder="Address" name="strAddress" id="strAddress" type="text" value="'.$strAddress.'" required disabled><br>';
