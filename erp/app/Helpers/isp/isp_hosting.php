                ...
                if (1 === $domain->transfer_in) {
                    if (is_local_domain($domain->domain)) {
                        ...
                        if (!empty($response['response'])) {
                            if ($domain->transfer_in > 0) {
                                ...
                                dbset('isp_host_websites', 'domain', $domain->domain, array('transfer_out' => 0));
                        ...
                        if (empty($response) || !is_array($response) || (isset($response['CommandResponse']['DomainTransferCreateResult']['@attributes']['Transfer']) && $response['CommandResponse']['DomainTransferCreateResult']['@attributes']['Transfer'] == 'false')) {
                            ...
                            dbset('isp_host_websites', 'domain', $domain->domain, array('transfer_in' => 2));
                            dbset('isp_host_websites', 'domain', $domain->domain, array('transfer_id' => $response['CommandResponse']['DomainTransferCreateResult']['@attributes']['TransferID']));
                            dbset('isp_host_websites', 'domain', $domain->domain, array('transfer_status' => $response['CommandResponse']['DomainTransferCreateResult']['@attributes']['StatusID']));
                        }
                    }
                    ...
                    if (is_local_domain($domain->domain)) {
                        ...
                        if ($response['response'] !== true) {
                            ...
                            dbset('isp_host_websites', 'domain', $domain->domain, array('transfer_status' => 1));
                            dbset('isp_host_websites', 'domain', $domain->domain, array('transfer_status_message' => $transfer_status));
                            ...
                            */
                            ...
                            dbset('isp_host_websites', 'domain', $domain->domain, array('transfer_in' => 0));
                            dbset('isp_host_websites', 'domain', $domain->domain, array('transfer_status' => 0));
                            dbset('isp_host_websites', 'domain', $domain->domain, array('transfer_status_message' => ''));
                        }
                    } else {
                        ...
                        dbset('isp_host_websites', 'domain', $domain->domain, array('transfer_status' => $status_id));
                        dbset('isp_host_websites', 'domain', $domain->domain, array('transfer_status_message' => $status_message));
                        if (5 == $status_id) {
                            dbset('isp_host_websites', 'domain', $domain->domain, array('transfer_in' => 0));
                            dbset('isp_host_websites', 'domain', $domain->domain, array('transfer_out' => 0));
                            ...
                            dbset('isp_host_websites', 'domain', $domain->domain, array('transfer_status' => $transfer_status));
                        }
                    }
                }
            }
    ...
    foreach ($domains as $domain) {
        ...
        if (0 == $hosted) {
            ...
            if (!$processed) {
                ...
                }elseif (true === $result['CommandResponse']['DomainCreateResult']['Registered'] || (isset($response['code']) && 1000 == $response['code']) || (isset($response['message']) && strpos($response['message'], 'exists') > -1)) {
                    dbset('isp_host_websites', 'domain', $domain->domain, array('to_register' => 0));
                    $account_id = $domain->account_id;
                    $function_variables = get_defined_vars();
                    $data['function_name'] = __FUNCTION__;
        ...
        if ('cloudtel6r2k8g' == $registrar_id) {
            ...
            if ($poll['response']['poll_contains_message']) {
                ...
                if ($zacr->poll_message_acknowledge($msg_id)) {
                    dbset('isp_host_zacr', 'id', $msg_id, array('acknowledged' => 1));
                }
...
function validate_domains()
{
    ...
    foreach ($domains as $domain_row) {
        ...
        if (is_local_domain($domain)) {
            ...
            if (1000 == $result['code']) {
                if ('cloudtel6r2k8g' == $result['data']['domain:infData']['domain:clID']) {
                    ...
                    $domain_data = [
                        ...
                        'auto_renew' => ('true' == $result['extension_data']['cozad:infData']['cozad:autorenew']) ? 1 : 0,
                        'provider' => 'zacr',
...
function siteworx_register($domain_name, $package, $account_id, $server = 'host2')
{
    ...
    if ((isset($result['status']) && 0 == $result['status']) || (isset($result['payload']) && strpos($result['payload'], 'Domain name already exists') > -1)) {
        return true;
    } else {
        return $result;
    }
...
function schedule_domains_delete_queue()
{
    ...
    foreach ($domains as $domain) {
        ...
        if ('zacr' == $domain->provider || (!$domain->provider && str_ends_with($domain->domain, '.za'))) {
