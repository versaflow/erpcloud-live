...
function button_menu_selected($module_id, $location, $grid_id, $event, $sub_items = false)
{
    ...
    */
    ...
    foreach ($buttons as $btn) {
        ...
        if ($btn->require_grid_id == 1 || ! empty($btn->grid_logic)) {
            if ($event == 'selected') {
...
function beforesave_check_menu_module($request)
{
    ...
    if (($request->menu_type == 'module' || $request->menu_type == 'module_form') && empty($request->module_id)) {
        return 'Module required.';
    }

    if ($request->menu_type == 'link' && empty($request->app_id)) {
        if ($request->location != 'grid_menu' && $request->location != 'related_items_menu') {
            return 'App ID required.';
        }
    }
    if ($request->menu_type != 'module' && $request->menu_type != 'module_form') {
