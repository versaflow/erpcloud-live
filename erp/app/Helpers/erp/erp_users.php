...
function afterdelete_employee_delete_payroll($request)
{
    ...
    \DB::table('acc_payroll_recon')->where('employee_id', $request->id)->update(['is_deleted' => 1]);
...
function aftersave_send_login_details($request)
{
    ...
    if (empty($user->role_id)) {
        ...
        if ($account->id == 1) {
            $role_id = '3';
        } elseif ($account->type == 'reseller_user' || $account->type == 'customer') {
            $role_id = '21';
        } elseif ($account->type == 'reseller') {
            $role_id = '11';
        }
