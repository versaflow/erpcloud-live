...
function sort_grid_rows($rows, $params)
{
    ...
    $sorts = array($params['orderby'] => $params['ordertype'], $params['orderby2'] => $params['ordertype2'], $params['orderby3'] => $params['ordertype3']);
...
function sql_filter_filter_users($rows)
{
    $filtered_rows = [];
    foreach ($rows as $row) {
        if ('rnd@telecloud.co.za' != $row->email) {
            continue;
        }
        $filtered_rows[] = $row;
    }
...
function replace_code_inserts($search, $replace)
{
    ...
    foreach ($tables as $table) {
        ...
        if (false !== strpos($table_name, $search)) {
    ...
    if ($field->field_type == 'select_module') {
        ...
        */
        ...
        if ('crm_pricelists'  == $field->opt_db_table) {
            ...
            if (in_array('partner_id', $columns) && 11 == session('role_id')) {
                $select_query->where('partner_id', $filter_val);
            } elseif (in_array('account_id', $columns) && 'Admin' != session('role_level')) {
...
function isIframeDisabled($src)
{
    try {
        ...
        if (!empty($headers['x-frame-options'])) {
            ...
            if ('ALLOW' == $x_options || $x_options == strtoupper('ALLOW-FROM http://portal.telecloud.co.za/')) {
                return false;
            } else {
                return true;
            }
