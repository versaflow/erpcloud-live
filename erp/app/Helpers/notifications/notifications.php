...
function erp_process_notification($account_id, $data = [], $function_variables = [], $conn = false)
{
    ...
    if (! empty($data['activation_email'])) {
    ...
    } elseif (isset($lookup_conn) && !empty($data['function_name'])) {
        // EVENT
        ...
        $notification = \DB::connection('default')->table('crm_email_manager')->where('id', $data['notification_id'])->get()->first();
    } elseif (isset($lookup_conn) && !empty($data['internal_function'])) {
        // FUNCTION
    ...
    if (isset($is_debtor_email) && !$is_debtor_email) {
    ...
    if (is_array($app_ids) && !in_array(14, $app_ids)) {
        $send_sms = false;
        $send_email = true;
    }
