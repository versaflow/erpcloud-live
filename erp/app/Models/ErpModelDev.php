...
class ErpModelDev extends Model
{
    ...
    public function validAccess()
    {
        ...
        if ($master_module_count) {
            $master_module_id = \DB::connection('default')->table('erp_cruds')->where('detail_module_id', $this->module->id)->pluck('id')->first();
            $mastermodule_access = \DB::connection('default')->table('erp_forms')->where('role_id', $grid_role_id)->where('module_id', $master_module_id)->get()->first();
        }

        if (empty($access)) {
            $access = (object) [];
        }
        if ($mastermodule_access) {
            $access->is_add = $module_access->is_add;
            $access->is_edit = $module_access->is_edit;
            $access->is_delete = $module_access->is_delete;
            $access->is_view = $mastermodule_access->is_view;
            $access->is_menu = $mastermodule_access->is_view;
        } elseif ($module_access) {
            $access->is_add = $module_access->is_add;
            $access->is_edit = $module_access->is_edit;
            $access->is_delete = $module_access->is_delete;
            $access->is_view = $module_access->is_view;
            $access->is_menu = $module_access->is_view;
        } elseif ($this->module->id) {
            $access->is_add = 0;
            $access->is_edit = 0;
            $access->is_delete = 0;
            $access->is_view = 0;
