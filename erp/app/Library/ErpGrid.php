...
class ErpGrid
{
    ...
    public function __construct($data, $params = [])
    {
        ...
        if (isset($this->data['module_fields'][0]['sort_order'])) {
            usort($this->data['module_fields'], array($this, 'sortGrid'));
        }
    }
            ...
            foreach ($this->data['module_fields'] as $i => $field) {
               ...
               // }
                ...
                if ($field_show) {
                    ...
                    } elseif ('decimal' == $field['field_type'] || 'percentage' == $field['field_type'] || 'integer' == $field['field_type']) {
                        $column['type'] = 'intField';
                ...
                if ($field_show && $field['id'] == $cell_style->field_id) {
                ...
                if ($field_show && $field['id'] == $row_style->field_id) {
                ...
                if('decimal' == $field['field_type'] || 'currency' == $field['field_type'] || 'percentage' == $field['field_type'] || 'integer' == $field['field_type']) {
                ...
                if($field['field_type'] == 'select_custom' || $field['field_type'] == 'select_function'){
                    ...
                    if('currency' == $db_field_type || 'integer' == $db_field_type) {
                        $column['headerClass'] = 'ag-right-aligned-header';
                        $column['cellClass'] .= ' ag-right-aligned-cell';
                    }
                }
        ...
        if (!empty($grid_view->global_default) || !empty($qs)) {
            ...
            foreach ($qs as $field => $value) {
                ...
                foreach ($this->data['module_fields'] as $i => $grid_field) {
                    ...
                    }else if($grid_field['field'] == $field) {
...
*/
    ...
    public function getGrid($grid_layout_id = false, $sub_grid = false)
    {
        ...
        $data['query_values'] = $grid_filters['query_values'];
        ...
        foreach ($this->data['module_fields'] as $i => $field) {
            ...
            if ($this->data['module_id'] != 588 && ('sort_order' == $field['field'] || 'menu_item_order' == $field['field'] || 'dialplan_order' == $field['field'] || 'dialplan_detail_order' == $field['field']) && session('role_level') == 'Admin') {
                $data['enable_sorting'] = true;
            }

            if ($field['groupby'] && $field_show) {
                $data['group_fields'][] = $field['field'];
            }

            if ($field['orderby'] && $field['orderby']!='select' && $field_show) {
                $data['sort_fields'][] = ['field' => $field['field'], 'direction' => ('asc' == $field['orderby']) ? 'ascending' : 'descending'];
            }
            ...
            if ($field_show) {
                ...
                if ('id' == $field['field']) {
                    ...
                    if ('currency' == $field['field_type'] || 'percentage' == $field['field_type'] || 'integer' == $field['field_type']) {
                        $column = [
                            'field' => $field['field'],
                            'headerText' => $field['label'],
                           // 'clipMode' => 'EllipsisWithTooltip',
                    ...
                    } elseif ('file' == $field['field_type'] || 'password_hover' == $field['field_type'] || 'phone_number' == $field['field_type'] || 'email' == $field['field_type'] || 'link' == $field['field_type']) {
                        $column = [
                            'field' => $field['field'],
                            'headerText' => $field['label'],
                ...
                if ('image' == $field['field_type'] || 'file' == $field['field_type'] || 'password_hover' == $field['field_type'] || 'phone_number' == $field['field_type'] || 'email' == $field['field_type'] || 'link' == $field['field_type']) {
                    $column['disableHtmlEncode'] = false;
                }
            }

            if ($field_show && $field['detail_visible']) {
                /*
                if ($frozen_columns == 0) {
                    $data['detail_columns'][] = [
                        'field' => $field['field'],
        ...
        $cols = collect($layout_data->columns);
        ...
        foreach ($col_chooser as $col) {
            foreach ($layout_data->columns as $i => $c) {
    ...
    public function formatGridData($rows)
    {
        ...
        if (!is_main_instance() && !check_access('1,31') && 'erp_users' == $this->data['db_table']) {
        ...
        foreach ($rows as $row) {
            ...
            foreach ($this->data['module_fields'] as $field) {
                ...
                if ('default' != $this->data['connection']) {
                    if ('account_id' == $field['field']) {
                        /*
            ...
            foreach ($this->data['module_fields'] as $i => $field) {
                ...
                if (!empty($get_filters)) {
                    foreach ($get_filters as $key => $val) {
                        ...
                        if ($field['field'] == $key) {
                            if ($this->data['module_id'] == 334 && 'product_id' == $key) {
                                $filter = (object) [
                                    'field' => $key,
                                ...
                                if (('reseller_user' == $key || str_ends_with($key, 'id')) && is_numeric($val)) {
