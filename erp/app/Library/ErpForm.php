...
class ErpForm
{
    ...
    public function __construct($data, $params = [])
    {
        ...
        if (isset($this->data['module_fields'][0]['sort_order'])) {
            usort($this->data['module_fields'], [$this, 'sortForm']);
        }
        $this->getFormId();
    ...
    {
        if ($this->data['db_key'] == 'id' || str_contains($this->data['db_key'], '_id')) {
        ...
        if (empty($id) && $request->from_menu == 1) {
    ...
    private function buildForm($tab = null, $import_form = false)
    {
        ...
        foreach ($this->data['module_fields'] as $field) {
            ...
            $add_to_html = ($tab == null || (! empty($field['tab']) && $field['tab'] == $tab) || (empty($field['tab']) && $tab == 'Superadmin')) ? true : false;
    ...
    private function getFormFieldValidation($field)
    {
        ...
        */
        ...
        } elseif ($field_type == 'currency' || $field_type == 'integer') {
    ...
    public function formAccess($field, $import_form = false)
    {
        ...
        if ($this->info['module_db'] == 'crm_accounts' && $this->row['id'] == session('account_id') && $field['field'] == 'pricelist_id') {
            return false;
        }

        if ($this->info['module_db'] == 'crm_accounts' && $this->row['id'] == session('account_id') && $field['field'] == 'status') {
            return false;
        }
    ...
    public function getFormField($field, $cell_editor = false)
    {
        ...
        extract($field);
        $conf = null;
        if (empty($default_value) && $field_type == 'date') {
            $default_value = '';
        }

        if (empty($default_value) && $field_type == 'datetime') {
            $default_value = '';
        }
        if (empty($default_value) && $field_type == 'time') {
            $default_value = '';
        }

        if (strtolower($default_value) == 'now' && $field_type == 'date') {
            $default_value = date('Y-m-d');
        }

        if (strtolower($default_value) == 'now' && $field_type == 'datetime') {
            $default_value = date('Y-m-d H:i:s');
        }

        if ($default_value == 'session_account_id') {
            $default_value = session('account_id');
            if ($this->data['module_id'] == 228 && ! empty(request()->account_id)) {
        ...
        || ($this->data['db_table'] == 'v_extensions' && $field == 'extension')) {
            $readonly = '';
            if ($this->data['edit_type'] == 'add') {
                $default_value = pbx_generate_extension(session('service_account_domain_uuid'), 1000);
            }

        }

        if (! empty($this->params->{$field}) && $this->data['edit_type'] == 'add' && $field != $this->data['db_key']) {
            $value = $this->params->{$field};
        } elseif ($default_value == 0 && $this->data['edit_type'] == 'add' && ($field_type == 'integer' || $field_type == 'currency')) {
            $value = $default_value;
        } elseif (! empty($default_value) && $this->data['edit_type'] == 'add') {
            $value = $default_value;
        } elseif ($row) {
            $value = $row[$field];
        } else {
        ...
        if ($field == 'user_id' && $this->data['module_id'] == 760 && $this->data['edit_type'] == 'add') {
        ...
        $readonly_val = '';
        if (($readonly == 'Add' || $readonly == 'Add and Edit') && $this->data['edit_type'] == 'add') {
            $readonly_val = 'readonly=true';
        }
        if (($readonly == 'Edit' || $readonly == 'Add and Edit') && $this->data['edit_type'] == 'edit') {
            $readonly_val = 'readonly=true';
        }

        if ($this->data['edit_type'] == 'view') {
            $readonly_val = 'readonly=true';
        }
        ...
        // db -> add field
        if ($this->data['module_id'] == 527 && $this->data['edit_type'] == 'add') {
                ...
                */
                ...
                if ($use_remote_data_binding) {
                    ...
                    ';
                        ...
                        if ($field_id == 'table_name527' || $field_id == 'table_name541') {
                            $this->data['form_script'] .= '
                        created: function(e){
                    ...
                    if (! empty($conf['account_filter'])) {
                        ...
                        if ($module_field['opt_db_table'] == 'crm_pricelists') {
                            ...
                            $columns = get_columns_from_schema($module_field['opt_db_table']);
                            if (in_array('partner_id', $columns) && session('role_id') != 21) {
                                $select_query->where('partner_id', $filter_val);
