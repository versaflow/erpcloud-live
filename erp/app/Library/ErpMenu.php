...
class ErpMenu{
            ...
            if($enable_workspace_menu_from_left_menu && $location == 'module_menu' && is_main_instance()){
                ...
                if(in_array($menu_params['module_id'],$workspace_ids)){
                    ...
                    if($workspace_render_id){
                        ...
                        $location = 'main_menu';
                        $is_workspace_menu = true;
                        $events_menus = Cache::rememberForever(session('instance')->id.'events_menus'.$workspace_render_id.$location, function() use ($location,$workspace_render_id) {
                            return \DB::connection('default')->table('erp_menu')
                            ->select('erp_menu.*')
                            ->where('location','main_menu')
                            ->where('erp_menu.unlisted',0)
                        ...
                        $link_menus = Cache::rememberForever(session('instance')->id.'linkmenus'.$workspace_render_id.$location, function() use ($location,$workspace_render_id) {
                            return \DB::connection('default')->table('erp_menu')
                            ->where('location','main_menu')
                            ->where('erp_menu.unlisted',0)
                            ->whereNull('erp_menu.module_id')
    ...
    public static function getAggridContextMenuAction($route_name, $button, $buttons)
    {
        ...
        if ('redirect' == $button->action_type) {
            $button_js .= '
			window.open(url);
			';
        }

        if ('ajax' == $button->action_type) {
            $button_js .= '
        ...
        if ('javascript' == $button->action_type) {
            $button_js .= '
        ...
        if ('transaction' == $button->action_type || 'form' == $button->action_type || 'view' == $button->action_type) {
            $height = 'auto';
            if ('transaction_modal' == $button->action_type) {
                $modal_type = 'transactionDialog';
            }
            if ('form' == $button->action_type) {
