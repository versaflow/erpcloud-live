...
class ErpMenu
{
    ...
    public static function build_menu($location, $menu_params = [], $render_module_id = false, $return_db_menu = false, $processed_descending_ids = [])
    {
        ...
        */
        ...
        if (is_array(session('app_ids')))
            $app_ids = session('app_ids');
        else
            $app_ids = array();

        if (session('role_level') == 'Admin' && in_array(2, $app_ids) ) {
    ...
    public static function format_menu_item_syncfusion($item, $menu_params, $location)
    {
        ...
        $menu_item->show_directly_on_toolbar = isset($item->show_directly_on_toolbar) ? $item->show_directly_on_toolbar : false;
        $menu_item->title = $item->menu_name;
        $menu_item->menu_title = $item->menu_name;
        $menu_item->url_params = isset($item->url_params) ? $item->url_params : '';
        $menu_item->navlink = strtolower(str_replace(' ', '-', $item->menu_name));
        $menu_item->submenu_function = isset($item->submenu_function) ? $item->submenu_function : '';
        ...
        $menu_item->menu_id = $item->id;
        $menu_item->button_function = isset($item->ajax_function_name) ? $item->ajax_function_name : '';

        $menu_item->cssClass = $location.'btn';

        if (isset($item->render_module_id) && $location != 'module_menu') {
        ...
        if (isset($item->module_id) && $item->module_id > 0 && empty($item->slug)) {
            $item->url = '#';
        }
        ...
        if (isset($item->action_type) && $item->action_type == 'view') {

            $menu_item->data_target = 'view_modal';
        }

        if (isset($item->module_id) && $item->module_id > '' && $item->menu_type != 'link' && $item->action_type == 'form') {
            $menu_item->data_target = 'sidebarform';
        } elseif ($item->menu_type == 'link' && isset($item->action_type) && $item->action_type == 'form') {
            $menu_item->data_target = 'form_modal';
        }
        if (isset($item->action_type) && $item->action_type == 'transaction') {
            $menu_item->data_target = 'transaction_modal';
        }
        if (isset($item->action_type) && $item->action_type == 'ajax' && ! empty($item->confirm_text)) {
        ...
        if (isset($item->action_type) && $item->action_type == 'javascript') {
        ...
        if (isset($item->action_type) && $item->action_type == 'form') {
            $menu_item->data_target = 'form_modal';
        }
        ...
        if ($account_id == 1 && is_main_instance() && isset($menu_item->module_id) && $menu_item->module_id != 512 && $menu_item->module_id != 343 && $menu_item->module_id != 348) {
            $account_id = 12;
        }
