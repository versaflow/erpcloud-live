...
class EldoBilling
{
    ...
    public function monthly_billing($docdate = false, $date_activated_bill = false)
    {
        ...
        $doc_ids = \DB::table('crm_documents')->where('docdate', $docdate)->where('doctype','Quotation')->where('reversal_id', 0)->where('billing_type', 'Monthly')->pluck('id')->toArray();
        ...
        foreach ($partners as $reseller) {
            ...
            */
            ...
            //     $total_taps = \DB::table('crm_rental_leases')->join('crm_rental_spaces','crm_rental_leases.rental_space_id','=','crm_rental_spaces.id')->where('crm_rental_leases.account_id', '>', 0)->where('crm_rental_spaces.account_id', '>', 0)->whereNotIn('crm_rental_spaces.id', $disabled_tenant_ids)->sum('taps');
            ...
            if ($partner_invoice['total'] > 0) {
                ...
                dbset('crm_documents', 'id', $partner_invoice['id'], array('tax' => currency($partner_invoice['tax']), 'total' => currency($partner_invoice['total'])));
            } else {
    ...
    public function monthly_billing_customers($reseller, $customer, $docdate)
    {
        ...
        */
        if (count($subscriptions) > 0) {
            ...
            foreach ($subscriptions as $sub) {
                ...
                */
                ...
                //council services
                if ($sub->product_id == 158) {
                    $summary['services'] += 450;
                    // $sub->qty = 1;
                }
                ...
                $qty = $sub->qty;
                if($qty > 0){
                    if ('Pending' == $customer->status and 11 == $sub->product_id) {
                        ...
                        $line['qty'] = $qty;
                        $line['description'] = $sub->detail;
                        $line['price'] = currency($sub->price_incl / 1.15);
        ...
        if ($invoice['total'] > 0) {
            ...
            dbset('crm_documents', 'id', $invoice['id'], array('tax' => currency($invoice['tax']), 'total' => currency($invoice['total'])));
        } else {
