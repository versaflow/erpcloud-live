...
@include('__app.grids.partials.charts', ['aggrid_charts' => $aggrid_charts,'module_id'=>$module_id,'grid_id'=>$grid_id])
@endif
...
function dropdowntargetrender(args){
 ...
 /** LAYOUT FUNCTIONS **/ 
    function layout_save{{ $grid_id }}(save_as_duplicate = false, type = 'default'){
        ...
        var data = {type: type,layout_type:window['layout_type{{$grid_id}}'], layout : layout, grid_reference: 'grid_{{ $grid_id }}', layout_id: window['layout_id{{ $grid_id }}'],pivot: pivot, pivot_mode: pivot_mode,  query_string: {!! $query_string !!},save_as_duplicate: save_as_duplicate};
        @if($master_detail && !$drill_down)
            if(detail_grid_api){
        ...
        if(window['layout_type{{$grid_id}}'] == 'Report'){
            ...
            data.chart_model = JSON.stringify(window['chartModel{{$grid_id}}']);
        }
        @endif
...
function getMainMenuItems{{ $grid_id }}(params) {
   ...
   // var menuItems = params.defaultItems.slice(0);
    ...
    menuItems.push({
    name: params.column.colDef.db_field,
    });
