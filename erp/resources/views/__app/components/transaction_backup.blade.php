...
<div class="row mb-0">
	...
	<div class="col">
		<div class="relative w-full text-left">
			<label class="flex text-sm not-italic items-center font-medium text-gray-800 whitespace-nowrap justify-between">
				<div>Contract Period <span class="text-sm text-red-500"> * </span></div>
				</label>
				<div class="flex flex-col mt-1">
					<div class="relative rounded-md shadow-sm font-base">
					<select name="contract_period" id="contract_period" class="contract_period font-base block w-full sm:text-sm border-gray-200 rounded-md text-black focus:ring-primary-400 focus:border-primary-400">
					<option value="0"></option>
		...
		<div class="table-responsive">
			...
			<tr class="clone box-border bg-white border border-gray-200 border-solid rounded-b">
				<td>
					...
					@if($is_supplier)
					<div class="e-input-group ledger_account-input-group">
					...
					<div class="cdr_destinations_div e-input-group">
...
//// Document Ready
	...
	$(document).ready(function() {
		...
		contract_period = new ej.dropdowns.DropDownList({
			allowFiltering: false,
			@if(!empty($document->contract_period))
			value: '{{$document->contract_period}}',
			@else
			value: '0',
			@endif
	        change: function(e){
	        	//console.log('contract_period change');
				update_product_prices();
	        },
		});
		contract_period.appendTo('#contract_period');
