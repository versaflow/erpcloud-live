...
<html>
<head>
<title>Telkom LTE Coverage</title>
...
$(document).off("click", "#coveragebtn").on("click", "#coveragebtn", function(e){
 ...
 $("#telkomcontainer").css('display','block');
 

 var coverage_url = '/check_lte_coverage';

 //console.log(coverage_url);
 	$.ajax({
			   url: coverage_url,
			   data: {provider: 'telkom',latlonginput:  $("#latlong-input").val(), addressinput:  $("#address-input").val()},
			   type: 'post',
      beforeSend: function() {
...
<script type="text/javascript" id="telkomscript">
 ...
 ax.async = true;
 ax.src = 'https://rcp.axxess.co.za/public/js/telkomLteJs.php?key='+mapApiKey+'&width='+mapWidth+'&height='+mapHeight;
 var s = document.getElementById('mapline');
 s.parentNode.insertBefore(ax, s);
 })();
</script>
<style>#telkommap{margin: 0 auto;}</style>
