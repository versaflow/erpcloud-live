...
$(document).off("click", "#coveragebtn").on("click", "#coveragebtn", function(e){
 ...
 var coverage_url = '/check_fibre_coverage?reseller=1';
 @else
 var coverage_url = '/check_fibre_coverage';
 @endif
 //console.log(coverage_url);
 	$.ajax({
			   url: coverage_url,
			   data: {latlonginput:  $("#latlong-input").val(), addressinput:  $("#address-input").val()},
			   type: 'post',
      beforeSend: function() {
       // $('#alertbox').html('<b>Loading available fibre products...');
  				 //  $('#alertbox').show();
      },
...
<hr id="mapline">
<div id="alertbox" class="my-2 p-4 d-none"></div>
</div>
