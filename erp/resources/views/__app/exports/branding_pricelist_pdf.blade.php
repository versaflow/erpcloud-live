...
<html lang="en">
  ...
  <body>
    ...
    <div style="text-align:right;"><b> All prices shown is Incl VAT. All services subject to minimum 3 months contract.</b></br></div>
...
@foreach($product_categories as $product_category)
    ...
    @if(!empty($pricelist_items[$product_category->id]) && count($pricelist_items[$product_category->id]) > 0)
      ...
      <th style="font-weight:bold;text-align:right;width:100px;">Monthly (Incl Vat)</th>
      <th style="font-weight:bold;text-align:right;width:100px;">12 Month Contract (Incl Vat)</th>
      <th style="font-weight:bold;text-align:right;width:100px;">Reseller Monthly (Incl Vat)</th>
      <!--<th style="font-weight:bold;text-align:right;width:100px;">Reseller 12 Month Contract</th>-->
      ...
      {!! strip_tags($pricelist_item->description,'<br>') !!}</td>
      <td style="text-align:right;">{{ $currency_symbol.''.currency($pricelist_item->price_tax) }} @if($pricelist_item->frequency == 'once off') <br>Once off @endif</td>
      <td style="text-align:right;">@if($pricelist_item->frequency == 'once off')  @else {{ $currency_symbol.''.currency($pricelist_item->price_tax_12) }} @endif</td>
      <td style="text-align:right">{{ $currency_symbol.''.currency($pricelist_item->reseller_price_tax) }}</td>
       <!--<td style="text-align:right">@if($pricelist_item->frequency == 'once off')  @else {{ $currency_symbol.''.currency($pricelist_item->reseller_price_tax_12) }} @endif</td>-->
