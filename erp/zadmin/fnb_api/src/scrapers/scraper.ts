...
export class Scraper {
	...
	private async _login() {
        ...
        this._page.setUserAgent('Mozilla/'+Math.floor(Math.random() * 90 + 10));

		await this._page.goto('https://fnb.co.za', {waitUntil: 'networkidle0'}
    
		await this._page.type('#user', this._options.username)
		await this._page.type('#pass', this._options.password)
    	await this._page.screenshot({path: 'ss-credentials.png',fullPage: true})
		// await this._page.click('input[type="submit"]')
		await this._page.click('#OBSubmit'); 
    	await this._page.screenshot({path: 'ss-submitted.png',fullPage: true})
		...
		await this._page.waitForFunction(() => !!document.querySelector('#loaderOverlay.overlayContainer.hide'))
