    ...
    var enableToolbarScrolling = function () { //enables scrolling if possible
        if (_this.toolbar.scrollWidth - _this.toolbar.clientWidth > 0) {
            ...
            var switchScrollArrows = function () {
                var maxWidth = __this.toolbar.scrollWidth - __this.toolbar.clientWidth;
                if (__this.toolbar.scrollLeft > 0 && !__this.leftScrollButton.classList.contains("fm-scroll-arrow")) {
                    changeListWrapperWidth("dec");
                    __this.addClass(__this.leftScrollButton, "fm-scroll-arrow");
                } else if (__this.toolbar.scrollLeft == 0 && __this.leftScrollButton.classList.contains("fm-scroll-arrow")) {
                    changeListWrapperWidth("inc");
                    __this.leftScrollButton.classList.remove("fm-scroll-arrow");
                }
                if (__this.toolbar.scrollLeft == maxWidth && __this.rightScrollButton.classList.contains("fm-scroll-arrow")) {
                    changeListWrapperWidth("inc");
                    __this.rightScrollButton.classList.remove("fm-scroll-arrow");
                } else if (__this.toolbar.scrollLeft < maxWidth && !__this.rightScrollButton.classList.contains("fm-scroll-arrow")) {
                    changeListWrapperWidth("dec");
                    __this.addClass(__this.rightScrollButton, "fm-scroll-arrow");
                }
            }
            ...
            var scrollList = function (direction) {
                var luft = 40;
                if (direction == "left") {
                    __this.toolbar.scrollLeft -= luft;
            ...
            __this.toolbar.onscroll = function () {
                switchScrollArrows();
            }
...
FlexmonsterToolbar.prototype.createTab = function (data) {
    ...
    tab.addEventListener("keyup", function (e) {
        if (e.which == 13 /*enter*/ || e.which == 32 /*space*/ ) {
            ...
            } else if (e.which == 32 /*space*/ ) { // enter calls click by default
                tabLink.click();
            }
        }
    });
    ...
    if (data.menu != null && (!this.osUtils.isMobile || data.collapse == true)) {
        var submenu = this.createTabMenu(data.menu);
...
FlexmonsterToolbar.prototype.showDropdown = function (elem) {
    ...
    if (menu) {
        ...
        if (menuRect.right > toolbarRect.right - arrowWidth) {
            ...
            var rowsSizes = document.querySelector("#fm-rows-resize");
            var luft = toolbarRect.left - menuRect.left + (rowsSizes ? rowsSizes.offsetWidth : 0);
            menu.style.left = luft + "px";
            var dropDownUl = menu.querySelector("ul.fm-dropdown-content");
            dropDownUl.classList.add("fm-dropdown-scrollable");
        }
    }
};
FlexmonsterToolbar.prototype.hideDropdown = function (elem) {
    ...
    if (menu) {
        ...
        this.removeClass(menu.querySelector("ul.fm-dropdown-content"), "fm-dropdown-scrollable");
    }
};
